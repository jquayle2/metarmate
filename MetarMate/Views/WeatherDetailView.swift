...
struct WeatherDetailView: View {
    ...
    // PilotNote and CrosswindDisplay now live in MetarPilotNotes.swift — extracted so the
    // (safety-critical) pilot-note derivation is a pure, unit-testable function, not a private
    // method on this View.
    ...
    private func pilotNotes(for metar: Metar, history: [Metar]) -> [PilotNote] {
        // Pure derivation lives in MetarPilotNotes.build; compute this View's runway-aware
        // crosswind displays here (@MainActor / RunwayService) and inject them as data so the
        // crosswind note keeps its per-runway detail while build stays pure and testable.
        MetarPilotNotes.build(metar: metar, history: history,
                              crosswindDisplays: crosswindDisplays(metar.wind))
    }
